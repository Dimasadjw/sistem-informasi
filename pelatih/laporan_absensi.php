...
if (isset($_POST['submit'])) {
    ...
    $ekstrakurikuler_id = $_POST['ekstrakurikuler'];
    ...
    if ($tanggal_mulai > $tanggal_selesai) {
        ...
        $query = "SELECT * FROM absensi a join tb_user b on a.id_user= b.id_user join tb_ekstra c on a.id_ekstra = c.id WHERE waktu_absen BETWEEN '$tanggal_mulai' AND '$tanggal_selesai' AND id_ekstra = '$ekstrakurikuler_id'";
        $result = mysqli_query($con, $query);

        while ($ray = mysqli_fetch_assoc($result)) {
            $data[] = $ray;
        }
    }
}
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped" id="dataTable" width="100%" cellspacing="0" style="color:black">
                                    ...
                                    <tbody>
                                        ...
                                        //         $query = "SELECT * FROM absensi WHERE waktu_absen BETWEEN '$tanggal_mulai' AND '$tanggal_selesai' AND id_ekstra = '$ekstrakurikuler_id'";
                                        //         $result = mysqli_query($con, $query);
                                        ...
                                        <!-- //     // Query data laporan jurnal berdasarkan tanggal dan ekstrakurikule
                                        //     if (!$result) {
                                        //         // Tampilkan pesan kesalahan jika query tidak berhasil dieksekusi
                                        //         echo "Error: " . mysqli_error($con);
                                        //     }
                                        // } else {
                                        //     // Tampilkan pesan jika data form tidak lengkap
                                        //     echo "Data form tidak lengkap. Pastikan semua kolom telah diisi.";
                                        // }
